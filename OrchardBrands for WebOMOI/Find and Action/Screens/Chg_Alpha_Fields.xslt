...
<!-- Search criteria:                            -->
<!--   where 'justification' is set to 'Left'    -->
<!--   where the flag 'StripBegin' is 'false'    -->
<!--   where the flag 'StripEnd' is 'false'      -->
<!-- Actions:                                    -->
<!--   set the flag 'StripPunctuation' to 'true' -->
...
<xsl:template match="ns:HostField">
  <xsl:choose>
      <xsl:when test="(count(./ns:Flags/ns:LeftAdjust) = 1) and (count(./ns:Flags/ns:StripBegin) = 1) and (count(./ns:Flags/ns:StripEnd) = 1)">
        <xsl:copy>
           ...
           <Flags>
             ...
             <xsl:apply-templates select="./ns:Flags/ns:StripBegin"/>
             <xsl:apply-templates select="./ns:Flags/ns:StripEnd"/>
             <StripPunctuation/>
